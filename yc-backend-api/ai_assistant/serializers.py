...
class AIAgentSerializer(serializers.ModelSerializer):
    ...
    def validate_max_tokens(self, value):
        ...
        if value < 1 or value > 100000:
            raise serializers.ValidationError("Max tokens must be between 1 and 100,000")
        return value
...
class ChatMessageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ChatMessage
        fields = [
            'id', 'chat_session', 'message_type', 'content', 'metadata',
...
class ChatSessionSerializer(serializers.ModelSerializer):
    ...
    ai_agent_provider = serializers.CharField(source='ai_agent.provider', read_only=True)

    class Meta:
        model = ChatSession
        fields = [
            'id', 'user', 'ai_agent', 'ai_agent_name', 'ai_agent_provider',
            'title', 'page', 'context', 'is_active', 'created_at', 'updated_at', 'messages'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at', 'ai_agent_name', 'ai_agent_provider']
...
class ChatSessionBasicSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user', 'ai_agent', 'ai_agent_name', 'ai_agent_provider',
            'title', 'page', 'context', 'is_active', 'created_at', 'updated_at', 'message_count'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at', 'ai_agent_name', 'ai_agent_provider']
...
class AIAgentUsageSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = AIAgentUsage
        fields = [
            'id', 'user', 'user_username', 'ai_agent', 'ai_agent_name',
            'date', 'total_messages', 'total_tokens', 'total_cost',
            'created_at', 'updated_at'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at', 'ai_agent_name', 'user_username']
...
class AIAgentConfigurationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'user', 'ai_agent', 'ai_agent_name', 'custom_temperature',
            'custom_max_tokens', 'system_prompt', 'is_favorite',
            'created_at', 'updated_at'
        ]
