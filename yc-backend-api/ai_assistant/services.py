...
class GeminiService(BaseAIService):
    ...
    def __init__(self, api_key: str, model_name: str = "gemini-pro"):
        ...
        try:
            ...
            raise AIServiceError("Google Generative AI library not installed. Run: pip install google-generativeai")
...
class CohereService(BaseAIService):
    ...
    def __init__(self, api_key: str, model_name: str = "command"):
        ...
        try:
            ...
            raise AIServiceError("Cohere library not installed. Run: pip install cohere")
    ...
    async def generate_response(
        ...
        **kwargs
        ...
        try:
            ...
            return {
                'response': response.text,
                'tokens_used': response.meta.tokens.input_tokens + response.meta.tokens.output_tokens if hasattr(response, 'meta') else None,
                'response_time_ms': response_time_ms,
                'metadata': {
                    'model': self.model_name,
...
class AIServiceFactory:
    ...
    def create_service(cls, provider: str, model_name: str) -> BaseAIService:
        ...
        if not api_key:
            raise AIServiceError(f"API key not found for {provider}. Set {api_key_var} in environment.")
