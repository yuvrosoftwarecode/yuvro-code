...
class StudentCodePracticeAdmin(admin.ModelAdmin):
    list_display = ["id", "user", "question_title", "status", "marks_obtained", "created_at"]
...
class UserCourseProgressAdmin(admin.ModelAdmin):
    list_display = ["id", "user", "course_name", "topic_name", "subtopic_name", "progress_percent", "is_completed", "is_current_subtopic", "last_accessed"]
    list_filter = ["is_completed", "is_current_subtopic", "is_videos_watched", "is_quiz_completed", "is_coding_completed", "course", "topic", "created_at"]
    search_fields = ["user__username", "user__email", "course__name", "topic__name", "subtopic__name"]
    ordering = ["-last_accessed", "-updated_at"]
    readonly_fields = ["id", "progress_percent", "created_at", "updated_at", "last_accessed"]
    ...
    fieldsets = (
        ...
        ("Component Progress", {
            "fields": ["is_videos_watched", "is_quiz_completed", "quiz_score", "is_coding_completed", "coding_score"],
            "classes": ("collapse",)
        }),
        ("Answers Data", {
            "fields": ["quiz_answers", "coding_answers"],
            "classes": ("collapse",)
        }),
        ("Metadata", {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('user', 'course', 'topic', 'subtopic')
    ...
    def recalculate_progress(self, request, queryset):
        ...
        self.message_user(request, f"Successfully recalculated progress for {updated_count} records.")
    ...
    def mark_as_current(self, request, queryset):
        ...
        UserCourseProgress.objects.filter(user_id__in=user_ids, is_current_subtopic=True).update(is_current_subtopic=False)
        
        # Then mark the selected ones as current
        updated_count = queryset.update(is_current_subtopic=True)
        
        self.message_user(request, f"Successfully marked {updated_count} subtopics as current.")
    ...
    def mark_as_completed(self, request, queryset):
        ...
        self.message_user(request, f"Successfully marked {updated_count} records as completed.")
