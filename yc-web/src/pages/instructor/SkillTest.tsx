...
const SkillTest: React.FC = () => {
    ...
    // Topic CRUD helpers (similar to CourseEdit)
    ...
    const saveTopic = async () => {
        ...
        try {
            if (topicModalMode === "create") {
                ...
                setTopics((prev) => prev.map((p) => (p.id === updated.id ? updated : p)));
        ...
        <div className="min-h-screen flex flex-col bg-gray-50">
            ...
            <div className="flex-1 overflow-hidden h-[calc(100vh-64px)]">
                <div className="h-full w-full p-4 md:p-6 flex gap-6">
                    {/* LEFT: Courses -> Topics */}
                    {/* LEFT: Collapsible Courses -> Topics (similar to CourseEdit) */}
                    <div
                                    ...
                                    {courses.map((course) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {/* Course Header */}
                                                <div
                                                    className={`p-3 flex items-center justify-between cursor-pointer ${expanded ? "bg-sky-50 border-sky-300" : ""
                                                        }`}
                                                    onClick={() =>
                                                        expanded
                                                        ...
                                                        {loadingTopics ? (
                                                            ...
                                                            <div className="space-y-2">
                                                                {topics.map((topic) => (
                                                                    <div
                                                                        key={topic.id}
                                                                        className={`p-2 rounded border flex items-center justify-between cursor-pointer ${selectedTopic?.id === topic.id
                    ...
                    <div className="bg-white rounded-md shadow-sm overflow-auto flex flex-col" style={{ flexBasis: "70%" }}>
                        <div className="border-b px-4 py-3 flex items-center justify-between">
                            <div className="flex gap-2">
                                <button className={`px-3 py-1 rounded-md text-sm ${rightTab === "mcqs" ? "bg-black text-white" : "text-gray-700 hover:bg-gray-100"}`} onClick={() => setRightTab("mcqs")}>MCQs</button>
                                <button className={`px-3 py-1 rounded-md text-sm ${rightTab === "coding" ? "bg-black text-white" : "text-gray-700 hover:bg-gray-100"}`} onClick={() => setRightTab("coding")}>Coding Problems</button>
                            </div>

                            <div className="text-sm text-muted-foreground">
                                {selectedTopic ? `Selected Topic: ${selectedTopic.name}` : "No topic selected"}
                            </div>
                        </div>
                        ...
                        <div className="p-6 flex-1 overflow-auto">
                            {!selectedTopic ? (
                                <div className="h-full flex flex-col items-center justify-center text-center text-muted-foreground">
                                    <div className="text-lg font-medium mb-2">Select a topic on the left</div>
                                    <div className="text-sm">Then use these tabs to manage MCQs and Skill Test coding problems</div>
                                </div>
                            ) : (
                                <div>
                                    {rightTab === "mcqs" && <SkillTestMCQs topic={selectedTopic} />}
                                    {rightTab === "coding" && <SkillTestCoding topic={selectedTopic} />}
                                </div>
                            )}
                        </div>
                    </div>
            ...
            <Dialog open={!!confirmDeleteTopicId} onOpenChange={() => setConfirmDeleteTopicId(null)}>
                <DialogContent>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setConfirmDeleteTopicId(null)}>Cancel</Button>
                        <Button variant="destructive" onClick={() => confirmDeleteTopicId && handleDeleteTopic(confirmDeleteTopicId)} disabled={deletingTopic}>
