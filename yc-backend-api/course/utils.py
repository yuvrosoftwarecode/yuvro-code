...
class CodeExecutionUtil:
    ...
    @staticmethod
    def execute_code(code, language, question_id=None, test_cases_basic=None, 
                    test_cases_advanced=None, test_cases_custom=None):
        """
    ...
    def check_plagiarism(code, language, question, user, service_url):
        ...
        try:
            ...
            recent_submissions = StudentCodePractice.objects.filter(
                question=question
            ).exclude(user=user).select_related('user').order_by('-created_at')[:20]
            
            for submission in recent_submissions:
                code_data = submission.answer_latest.get('code', '') if submission.answer_latest else ''
    ...
    @staticmethod
    def create_or_update_practice_record(user, question, code, language, 
                                       execution_results, execution_output, 
                                       test_results, course=None, topic=None):
        """
        ...
        practice_record, created = StudentCodePractice.objects.get_or_create(
            ...
            defaults={
                ...
                'topic': topic or question.topic or (question.subtopic.topic if question.subtopic else None),
        ...
        if test_results:
            ...
            if total_tests_count > 0:
                practice_record.marks_obtained = (passed_tests_count / total_tests_count) * 100
