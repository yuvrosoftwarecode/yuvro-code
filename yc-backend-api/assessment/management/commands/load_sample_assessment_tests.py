    ...
    def load_skill_tests_from_json(self, data):
        ...
        for skill_test_data in skill_tests:
            try:
                ...
                self.stdout.write(self.style.ERROR(f'Error loading skill test "{skill_test_data.get("title", "Unknown")}": {str(e)}'))
    ...
    def load_contests_from_json(self, data):
        ...
        for contest_data in contests:
            try:
                ...
                self.stdout.write(self.style.ERROR(f'Error loading contest "{contest_data.get("title", "Unknown")}": {str(e)}'))
        ...
        for mock_interview_data in mock_interviews:
            try:
                ...
                mock_interview_kwargs = {
                    ...
                    'max_duration': mock_interview_data.get('max_duration', mock_interview_data.get('duration', 45)),
                    ...
                    'interviewer_name': mock_interview_data.get('interviewer_name', 'Junnu'),
                    'interviewer_voice_id': mock_interview_data.get('interviewer_voice_id', ''),
                ...
                if not existing_mock_interview:
                    ...
                    self.stdout.write(f'↻ Updated existing mock interview: {mock_interview_data["title"]}')
                    
            except Exception as e:
                self.stdout.write(self.style.ERROR(f'Error loading mock interview "{mock_interview_data.get("title", "Unknown")}": {str(e)}'))