...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        json_file_path = os.path.join(os.path.dirname(__file__), '../../fixtures/sample_jobs_data.json')
        ...
        self.stdout.write(self.style.SUCCESS('Successfully loaded all sample companies and jobs from JSON!'))
    ...
    def load_jobs_from_json(self, data):
        ...
        for job_data in data.get('jobs', []):
            self.stdout.write(f'Loading job: {job_data["title"]} at {job_data["company_name"]}')
            
            company_name = job_data.get('company_name')
            company = company_map.get(company_name)
            
            if not company:
                self.stdout.write(self.style.WARNING(f'Company {company_name} not found, skipping job'))
                continue
            ...
            existing_job = Job.objects.filter(
                title=job_kwargs['title'],
                company=job_kwargs['company']
            ).first()
            
            if not existing_job:
                Job.objects.create(**job_kwargs)
                self.stdout.write(f'✓ Created job: {job_data["title"]} at {company_name}')
            else:
                self.stdout.write(f'→ Job already exists: {job_data["title"]} at {company_name}')